...
export function computeProtettiviCart(
  ...
  if (isH2O) {
    if (line !== 'LAMINE' && line !== 'CORLITE') {
      ...
      lines.push({ sku_id: fixSku, descrizione: withZone(descOf(store, fixSku)), qty: fixQty, prezzo_unitario: priceOf(store, fixSku), totale: fixQty * priceOf(store, fixSku), section: 'protettivi' });
    ...
    if (line === 'LAMINE') {
      ...
      lines.push({ sku_id: lucidoSku, descrizione: withZone(`${descOf(store, lucidoSku)} — LAMINE base a spatola`), qty: lucidoQty, prezzo_unitario: priceOf(store, lucidoSku), totale: lucidoQty * priceOf(store, lucidoSku), section: 'protettivi' });
      ...
      if (finitura === 'OPACO') {
        ...
        lines.push({ sku_id: opSku, descrizione: withZone(descOf(store, opSku)), qty: opQty, prezzo_unitario: priceOf(store, opSku), totale: opQty * priceOf(store, opSku), section: 'protettivi' });
    ...
    if (line === 'CORLITE') {
      if (finitura === 'LUCIDO') {
        ...
        lines.push({ sku_id: waxSku, descrizione: withZone('Seal Wax lucida (2 mani, 30 g/m²/mano)'), qty: waxQty, prezzo_unitario: priceOf(store, waxSku), totale: waxQty * priceOf(store, waxSku), section: 'protettivi' });
    ...
    if (opaco_colorato && finitura === 'PROTEGGO_COLOR_OPACO') {
      ...
      lines.push({ sku_id: colorSku, descrizione: colorAndZone('PROTEGGO COLOR'), qty: colorQty, prezzo_unitario: priceOf(store, colorSku), totale: colorQty * priceOf(store, colorSku), section: 'protettivi' });
      ...
      lines.push({ sku_id: transparentSku, descrizione: withZone(`${descOf(store, transparentSku)} (mano finale trasparente obbligatoria)`), qty: transQty, prezzo_unitario: priceOf(store, transparentSku), totale: transQty * priceOf(store, transparentSku), section: 'protettivi' });
    ...
    if (finitura === 'LUCIDO') {
      ...
      lines.push({ sku_id: lucSku, descrizione: withZone(descOf(store, lucSku)), qty: lucQty, prezzo_unitario: priceOf(store, lucSku), totale: lucQty * priceOf(store, lucSku), section: 'protettivi' });
  ...
  if (!isH2O) {
    ...
    if (line === 'LAMINE') {
      ...
      lines.push({ sku_id: lucSku, descrizione: withZone(`${descOf(store, lucSku)} — LAMINE base a spatola`), qty: lucQty, prezzo_unitario: priceOf(store, lucSku), totale: lucQty * priceOf(store, lucSku), section: 'protettivi' });
    ...
    if (opaco_colorato) {
      ...
      lines.push({ sku_id: premixSku, descrizione: withZone('Premix colore Opaco S (1 per confezione)'), qty: opQty, prezzo_unitario: priceOf(store, premixSku), totale: opQty * priceOf(store, premixSku), section: 'protettivi' });
